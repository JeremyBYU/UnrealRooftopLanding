...
def main(gui=True, segmented=False):
    ...
    for record in records['records']:
        ...
        t2 = time.perf_counter()
        conf_map_comb = create_confidence_map_combined(conf_map_seg, conf_map_plan)

        img_scene = np.concatenate((img_scene, img_seg), axis=1)
        conf_map_seg_color = cm.viridis(conf_map_seg)[:, :, :3][..., ::-1].copy()
        conf_map_plan_color = cm.viridis(conf_map_plan)[:, :, :3][..., ::-1].copy()
        conf_map_comb_color = cm.viridis(conf_map_comb)[:, :, :3][..., ::-1].copy()
